import os
#------wsy add----------------------------
# os.environ['CUDA_VISIBLE_DEVICE']="6,4"
#-----------------------------------------
import argparse
...
def process(filename):
    ...
    if not os.path.exists(save_name):
        devive = torch.device("cuda:6" if torch.cuda.is_available() else "cpu")
        ...
        c = torch.load(save_name,map_location=torch.device('cuda:6'))
...
if __name__ == "__main__":
    ...
    hmodel = utils.get_hubert_model(6 if torch.cuda.is_available() else None)
    #---------------------------------------------------------------------------
    print("Loaded hubert.")
